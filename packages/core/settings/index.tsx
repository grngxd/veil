...
import { renderPreactInReact } from '../react';
...
const PreactCounter = () => {
    ...
    return (
        <div>
            <h2>skibidi preact counter</h2>
            <button onClick={() => setCount(count + 1)} style={{ padding: '8px' }}>
                Count: {count}
            </button>
        </div>
    );
...
export const init = () => {
    getDispatcher().waitForDispatch("USER_SETTINGS_MODAL_OPEN").then(() => {
        log("settings init");
        abuseWebpack((c) => {
                        ...
                        chunk.exports.ZP.prototype.getPredicateSections, 
                        {
                            apply: (target, thisArg, args) => {
                                ...
                                if (customElements.length > 0) {
                                    result.splice(insertIndex, 0, ...customElements, { section: "DIVIDER" });
                                }
